...
 
    for (int i = 1; i <= n; i++)
    {
        if (arr[i] != arr[i - 1])
        {
            mapa[arr[i - 1]] = max(mapa[arr[i - 1]], cnt);
            cnt = 1;
        }
        else
            cnt++;
...
 
    for (int i = 1; i <= n; i++)
    {
        if (brr[i] != brr[i - 1])
        {
            mapb[brr[i - 1]] = max(mapb[brr[i - 1]], cnt);
            cnt = 1;
        }
        else
            cnt++;
...
 
    if (cnt > 0)
        mapb[brr[n]] = max(mapb[brr[n]], cnt);
 
    int ans = 1;
 
    for(auto [val , freq] : mapa) ans = max(ans , freq + mapb[val]);
    for(auto [val , freq] : mapb) ans = max(ans , freq + mapa[val]);
