...
 
    ...
    for (auto &i : arr)
        cin >> i;
 
    int tsum = accumulate(begin(arr), end(arr), 0LL , [](int val,int ele){
        return  abs(ele) + val;
    });
    int mini = abs(*min_element(begin(arr), end(arr), [](auto &a, auto &b)
                            { return abs(a) < abs(b); }));
 
    int negcnt = count_if(begin(arr),end(arr),[](int x){
        return x < 0;
    });
    debug(tsum);
    debug(mini);
